...
The Burgess Principle is the ethical and architectural root from which OpenHear grows. It is a framework, authored by Lewis James Burgess, for sovereign handling of clinical audiology data — audiograms, fittings, MPO calculations — so that a patient, a clinician, and a regulator can each hold an independent, verifiable, offline record of what was committed to and by whom, without depending on the cooperation of any vendor.

It rests on five commitments, enforced in this repository (see `advocacy/gate.py`, `advocacy/adapters.py`, and `NOTICE`):

1. **Sovereign facts belong to the person they describe.** The audiogram and fitting profile are facts about a human body and are never transmitted, bundled, or shared without that person's explicit, informed consent.
2. **Every clinical commitment is cryptographically verifiable.** A SHA-256 commitment over a canonical serialisation of the facts is the only value intended to leave the device, and any party holding the bundle can verify it offline with no vendor involvement.
3. **Trust is binary: `SOVEREIGN` or `NULL`.** A real human personally reviewing the specific facts and signing a receipt produces `SOVEREIGN`; pure automation, blanket policy, an invalid signature, tampering, or a refusal to engage produces `NULL`. There is no third state.
4. **Raw audio is not a clinical fact.** Any attempt to commit `bytes`, `bytearray`, `memoryview`, or a NumPy `ndarray` into a facts payload raises `RawAudioRejectedError` before a commitment is produced.
5. **Schema ownership is singular.** The `openhear-advocacy-v1` schema family is owned by this repository; integrators do not fork, re-version, or extend it.

The advocacy layer in this repository is the reference implementation of these commitments. The next layer — **Iris**, an AI companion built within the Burgess Principle — will provide a higher-level advocacy workflow post-1.0.0. Iris will import OpenHear; OpenHear will not import Iris. An integration that satisfies the five commitments today will be compatible with Iris automatically.

The full statement of the principle, the integrator contract, and the runnable reference integration are in [`docs/BURGESS_PRINCIPLE.md`](docs/BURGESS_PRINCIPLE.md), [`docs/ADVOCACY_INTEGRATION.md`](docs/ADVOCACY_INTEGRATION.md), [`docs/INTEGRATORS.md`](docs/INTEGRATORS.md), and [`examples/reference_integration.py`](examples/reference_integration.py).
...
OpenHear is built on **the Burgess Principle** (summarised above and stated in full in [`docs/BURGESS_PRINCIPLE.md`](docs/BURGESS_PRINCIPLE.md)) — the position that data generated by or about your body belongs to you and not to the institution that measured it, and that any clinical commitment about your body must be independently verifiable, offline, by anyone holding the bundle.

The advocacy layer in this repository is the reference implementation of that principle. The principle in turn sits within a wider body of work on data sovereignty by the same author, including [the legal framing repository](https://github.com/ljbudgie/Burgessprinciple) on void ab initio against bulk-processed warrants and tainted data records.

OpenHear applies the principle directly to audiological data:
...
Phase 6 adds local listener preference capture, deterministic adaptive config
suggestions, and per-environment saved profiles. A May 2026 coverage pass
confirms the Python suite exercises the core DSP, audiogram, advocacy,
wristband, stream, hardware safety, and Phase 5 sovereign-device paths with
branch coverage enabled. The lowest-coverage gaps found in that pass were the
live wristband runtime loop and micro:bit firmware main-loop/fallback branches;
both now have deterministic tests using local hardware stubs, so mobile-native
work can start from a stronger regression baseline without requiring BLE, audio
devices, or a micro:bit in CI.
